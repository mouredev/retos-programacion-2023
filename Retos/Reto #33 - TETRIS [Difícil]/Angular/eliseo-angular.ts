...
import { pieza1, pieza2, emptyBoard, blankLine } from './pieces';
import {
  fromEvent,
  Subscription,
  timer,
...
@Component({
  ...
  <div class="piece">
    <ng-container *ngFor="let row of nextPiece.data[0]">
      <div
...
export class App implements OnInit, OnDestroy {
  ...
  //"tablero"
  board: any[] = [];
  pieces = [pieza1, pieza2, pieza1, pieza2, pieza1];
  ...
  private check(rotation: number, posX: number, posY: number) {
    for (let i = 0; i < this.piece.data[rotation].length; i++) {
      for (let j = 0; j < this.piece.data[rotation][i].length; j++) {
        if (this.piece.data[rotation][i][j] && this.board[posY + i][posX + j])
          return false;
      }
    }
    return true;
  ...
  private checkDeleteLines() {
    ...
    const deleteLines = this.piece.data[this.rotation]
      .map((_, i: number) => {
        return this.board[this.posY + i].find((x: any) => !x) === null
          ? -1
          : this.board[this.posY + i][1] != 'silver'
          ? this.posY + i
          : -1;
      })
      .filter((x: number) => x > -1);
  ...
  private fillBoard() {
    this.piece.data[this.rotation].forEach((row: number[], i: number) => {
      row.forEach((x: number, j: number) => {
        if (x) this.board[this.posY + i][this.posX + j] = this.piece.color;
      });
    });
  }
  ...
  private removeBoard() {
    this.piece.data[this.rotation].forEach((row: number[], i: number) => {
      row.forEach((x: number, j: number) => {
        if (x) this.board[this.posY + i][this.posX + j] = null;
      });
    });
  }
