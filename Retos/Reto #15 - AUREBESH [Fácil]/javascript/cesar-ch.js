...
function translate(text) {
    ...
    let i = 0;
    while (i < text.length) {
        if ( basicAlphabet[text.slice(i, i + 2)]) {
            translated_text += basicAlphabet[text.slice(i, i + 2)];
            i += 2;
        } else {
            translated_text += basicAlphabet[text[i].toLowerCase()] || text[i];
            i += 1;
        }
    }
...
console.log(translate("Brais"))
console.log(translate("Mouredev"))
