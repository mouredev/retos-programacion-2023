...
def dibujar_pista(obstaculos, id_player, player_pos):
    pista = []
    for p in range(num_pistas):
        ...
        player_simbol = COLISION if player_pos[p] in obstaculos[p] else PLAYER[id_player[p]]
        
        track[player_pos[p]] = player_simbol
        
        pista.append(track)
        print(''.join(track))
    
    
    indices = [i for i, x in enumerate(player_pos) if x == min(player_pos)]
    if len(indices) > 1:
...
def generar_movimiento(player_position, obstaculos, player_crash):
    ...
    for p in range(num_pistas):
        ...
        if player_crash[p]:
            ...
            if new_position[p] in obstaculos[p]:
                player_crash[p] = True
